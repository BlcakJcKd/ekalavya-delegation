...
def _summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {"runs": len(rows), "successful_runs": success, "failed_runs": failed, "aborted_runs": aborted, "success_rate": success / len(rows) if rows else None, "reported_tokens": _metric(rows, "total_tokens"), "input_tokens": _metric(rows, "input_tokens"), "output_tokens": _metric(rows, "output_tokens"), "reasoning_tokens": _metric(rows, "reasoning_tokens"), "cached_input_tokens": _metric(rows, "cached_input_tokens"), "latency_seconds": {"median": _quantile(walls, .5) if len(walls) >= 3 else None, "p90": _quantile(walls, .9) if len(walls) >= 10 else None, "available": len(walls), "p90_minimum_sample": 10}, "feedback": {"available": len(feedback), "unrated": len(rows) - len(feedback), "counts": feedback_counts, "useful_rate": feedback_counts["useful"] / len(feedback) if feedback else None}}
