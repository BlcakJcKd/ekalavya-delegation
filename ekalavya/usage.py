...
def build_usage(conn: Any, *, period: str = "7d", by: str | None = None, filters: dict[str, str | None] | None = None, quotas: list[dict[str, Any]] | None = None, _now: datetime | None = None) -> dict[str, Any]:
    ...
    return {"schema_version": SCHEMA_VERSION, "scope": {"local_history": "only Ekalavya-observed delegations on this machine", "account_quota": "provider-reported only when a safe adapter exists", "native_same_provider_activity": "outside local history", "direct_provider_web_desktop_activity": "outside local history", "external_machine_activity": "outside local history", "outside_ekalavya_activity": "outside local history"}, "period": {"label": label, "start_utc": start.isoformat() if start else None, "end_utc": end.isoformat()}, "coverage": {"observability_events": {"available": len(rows), "eligible": len(rows)}, "token_telemetry": {"available": sum(r.get("total_tokens") is not None for r in rows), "eligible": len(rows)}, "reasoning_tokens": {"available": sum(r.get("reasoning_tokens") is not None for r in rows), "eligible": len(rows)}}, "summary": _summary(rows), "groups": _groups(rows, by), "quota": quotas or [], "warnings": ["Local Ekalavya history is not total provider-account usage; it excludes activity outside this machine and Ekalavya.", "Cross-provider token counts are reported units, not directly comparable efficiency measures."]}
